-- Exclude archived projects from get_task_analytics RPC
-- Previously, the function included tasks from archived projects in all analytics
...
COMMENT ON FUNCTION get_task_analytics(TEXT, UUID) IS
  'Aggregates all 10 task analytics metrics for TaskBoard dashboard. Excludes archived projects. Filters by company_id for security. Returns single row with 32 columns. Performance target: < 500ms for 1000 tasks.';
