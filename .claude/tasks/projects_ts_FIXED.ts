...
/**
 * Get user's company and role from NextAuth session
 * This replaces the old getUserCompanyAndRole that used supabase.auth.getUser()
 */
async function getUserContext() {
  // Get NextAuth session (NOT Supabase auth)
  ...
  if (companyError || !companyUser) {
    console.error('Company user query error:', companyError);
...
export async function createProject(formData: FormData) {
  ...
  const { userId, companyId, role } = userContext;
...
export async function updateProject(formData: FormData) {
  ...
  const { companyId, role } = userContext;
