...
async function getUserContext() {
  ...
  // Only GC Admin can manage phase templates
  if (companyUser.role !== 'gc_admin') {
    return { error: 'Insufficient permissions. Only GC Admin can manage phase templates.' };
  }
...
export async function createPhaseTemplate(formData: FormData): Promise<{
  ...
  if (error) {
    ...
    if (error.code === '23505') {
      return { error: 'A phase template with this name already exists for this project type' };
    }
    return { error: 'Failed to create phase template' };
  }

  console.log('[createPhaseTemplate] Phase template created:', phaseTemplate.id);
...
export async function updatePhaseTemplate(
  ...
  console.log('[updatePhaseTemplate] Phase template updated:', phaseTemplate.id);
  revalidatePath('/app/settings');
  return { success: true, phaseTemplate };
}
...
export async function reorderPhaseTemplates(
  ...
  console.log('[reorderPhaseTemplates] Reordering phases for project type:', projectTypeConfigId);
