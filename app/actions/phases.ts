...
import type { PhaseStatus, TaskPriority, TaskStatus } from "@/types/db/enums";
...
async function verifyPhaseAccess(
  ...
  const project = phase.projects as unknown as {
    id: string;
    company_id: string;
  };
...
export async function startNextPhase(projectId: string) {
  ...
  const { data: nextPhase, error: phaseError } = await supabase
    .from("project_phases")
    .select(
      "id, project_id, name, order_index, completion_percentage, started_at, completed_at, notes, created_at, updated_at, status",
    )
    .eq("project_id", projectId)
 ...
 */
export async function completeCurrentPhase(
  projectId: string,
...
async function checkProjectPhasePermission(
  ...
  return {
    hasPermission: false,
    error:
      "Insufficient permissions. Only Admin and Project Manager can manage phases.",
  };
}
...
export async function deletePhase(
  ...
  if (taskHandling === "move") {
    ...
    if (!targetPhase) {
      return {
        error: "Target phase not found or does not belong to the same project",
      };
    }
...
export async function applyTaskTemplates(
  ...
  console.log(
    "[applyTaskTemplates] Applying task templates to phase:",
    phaseId,
  );
  ...
  const [phaseTemplatesResult, existingTasksResult] = await Promise.all([
    supabase
      ...
      .eq("name", phase.name)
      .eq("is_active", true),
    supabase.from("tasks").select("title").eq("phase_id", phaseId),
  ]);

  const { data: phaseTemplates, error: phaseTemplatesError } =
    phaseTemplatesResult;
  if (phaseTemplatesError) {
    console.error(
      "[applyTaskTemplates] Error finding phase template:",
      phaseTemplatesError,
    );
  ...
  if (!phaseTemplates || phaseTemplates.length === 0) {
    return {
      error: `No phase template found matching "${phase.name}" with task templates`,
    };
  }

  // Pick the phase template with the most task templates
  const bestTemplate = phaseTemplates.reduce((best, current) => {
    const currentCount = Array.isArray(current.task_templates)
      ? current.task_templates.length
      : 0;
    const bestCount = Array.isArray(best.task_templates)
      ? best.task_templates.length
      : 0;
    return currentCount > bestCount ? current : best;
  });

  console.log(
    "[applyTaskTemplates] Using phase template:",
    bestTemplate.id,
    "with",
    Array.isArray(bestTemplate.task_templates)
      ? bestTemplate.task_templates.length
      : 0,
    "task templates",
  );
  ...
  const { data: taskTemplates, error: templatesError } = await supabase
    .from("task_templates")
    .select(
      "id, company_id, phase_template_id, title, description, default_task_type, default_priority, order_index, is_active, created_at, updated_at, days_offset",
    )
  ...
  if (templatesError) {
    console.error(
      "[applyTaskTemplates] Error fetching task templates:",
      templatesError,
    );
  ...
  const tasksToCreate = taskTemplates
    ...
    .map((template) => {
      ...
      if (
        template.days_offset !== null &&
        template.days_offset !== undefined &&
        project.start_date
      ) {
