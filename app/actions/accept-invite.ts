...
export async function validateInvitationToken(
  ...
  try {
    ...
    // Find team_invitations entry with this token (include company details)
    const { data: invitation, error: invitationError } = await supabase
      ...
      .select(
        ...
        expires_at,
        used_at,
        companies!team_invitations_company_id_fkey (
          id,
          name
        )
      `,
    ...
    if (inv.used_at !== null) {
      return {
        success: false,
        error:
          "This invitation has already been used. If you need access, please contact your administrator.",
      };
    }
    ...
    if (!company) {
      console.error("Error fetching company: missing company relation");
  ...
  try {
    ...
    if (!session?.user?.id || !session?.user?.email) {
      return {
        success: false,
        error:
          "You must be signed in to accept an invitation. Please sign in and try again.",
      };
    }
    ...
    if (invitation.email.toLowerCase() !== authenticatedEmail) {
      return {
        success: false,
        error:
          "This invitation is for a different email address. Please sign out and sign in with the invited email address.",
      };
    }

    // Use admin client for atomic operations
    const supabase = createAdminClient() as unknown as ReturnType<
    ...
    const { data: markedInvitation, error: markError } = await (
      supabase.from("team_invitations") as any
    )
      .update(updateData)
    ...
    if (markError || !markedInvitation) {
      ...
      return {
        success: false,
        error:
          "This invitation has already been used or has expired. Please contact your administrator.",
      };
    }
    ...
    console.log(
      "[ACCEPT_INVITE] Creating/updating user profile for user:",
      authenticatedUserId,
    );
    ...
    if (profileError) {
      console.error(
        "[ACCEPT_INVITE] Error creating/updating user profile:",
        profileError,
      );
      return {
        success: false,
    ...
    console.log(
      "[ACCEPT_INVITE] User profile created/updated successfully:",
      profileData,
    );
    ...
    if (existingMember) {
      ...
      if (member.status === "active") {
        return {
          success: false,
          error: "You are already an active member of this company.",
        };
      }
      ...
      console.log("[ACCEPT_INVITE] Creating company_users entry for:", {
        userId: authenticatedUserId,
        companyId: invitation.companyId,
        role: invitation.role,
      });
      const { data: companyUserData, error: createError } = await (
        supabase.from("company_users") as any
      )
        .insert({
      ...
      if (createError) {
        console.error(
          "[ACCEPT_INVITE] Error creating company user:",
          createError,
        );
        console.error(
          "[ACCEPT_INVITE] Error details:",
        ...
        return {
          success: false,
          error: `Failed to activate account: ${createError.message}`,
        };
      }

      console.log(
        "[ACCEPT_INVITE] Company user created successfully:",
        companyUserData,
      );
    }
