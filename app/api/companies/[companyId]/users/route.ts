...
export async function GET(
  ...
  try {
    ...
    console.log(
      "[GET /api/companies/:companyId/users] User ID:",
      session.user.id,
    );
    ...
    if (companyError) {
      console.error(
        "[GET /api/companies/:companyId/users] Company user query error:",
        companyError,
      );
    ...
    if (!companyUser) {
      console.error(
        "[GET /api/companies/:companyId/users] User not part of company",
      );
      return NextResponse.json({ error: "Unauthorized" }, { status: 403 });
    }

    console.log(
      "[GET /api/companies/:companyId/users] User role in company:",
      companyUser.role,
    );
    ...
    if (usersError) {
      console.error(
        "[GET /api/companies/:companyId/users] Users query error:",
        usersError,
      );
    ...
    console.log(
      "[GET /api/companies/:companyId/users] Found users:",
      companyUsers?.length || 0,
    );

    // Transform data to include user profiles
    const users = (companyUsers || [])
      .filter((cu: any) => cu.user_profiles)
      .map((cu: any) => ({
    ...
    console.log(
      "[GET /api/companies/:companyId/users] Returning users:",
      users.length,
    );
    ...
    console.error(
      "[GET /api/companies/:companyId/users] Unexpected error:",
      error,
    );
    return NextResponse.json(
