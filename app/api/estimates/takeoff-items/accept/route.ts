...
export async function POST(request: NextRequest) {
  try {
    ...
    if (updateError || !updatedItem) {
      console.error("[takeoff-items/accept] Update error:", updateError);
      return NextResponse.json(
        { error: "Failed to accept item or item not found" },
        { status: 404 },
      );
    }
    ...
    console.error("[takeoff-items/accept] Unexpected error:", error);
    return NextResponse.json(
      {
