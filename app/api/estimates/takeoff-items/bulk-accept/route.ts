...
export async function POST(request: NextRequest) {
  try {
    ...
    if (updateError) {
      console.error("[takeoff-items/bulk-accept] Update error:", updateError);
      return NextResponse.json(
    ...
    console.error("[takeoff-items/bulk-accept] Unexpected error:", error);
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Failed to accept items",
      },
      { status: 500 },
    );
  }
