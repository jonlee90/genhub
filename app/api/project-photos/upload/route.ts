...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (photoError) {
      console.error(
        "[POST /api/project-photos/upload] Storage error:",
        photoError,
      );
    ...
    if (thumbError) {
      console.error(
        "[POST /api/project-photos/upload] Thumbnail storage error:",
        thumbError,
      );
    ...
    } = supabase.storage.from("project-files").getPublicUrl(photoPath);
    ...
    if (insertError) {
      console.error(
        "[POST /api/project-photos/upload] Insert error:",
        insertError,
      );
