  ...
  if (!analytics) {
    return (
      <div className="bg-white rounded-xl overflow-hidden border-2 border-gray-200 shadow-sm p-6 text-center">
        ...
        <p className="text-sm font-medium text-gray-600">Unable to load expense summary</p>
      </div>
    );
  }

  // Calculate metrics
  const approvalRate = analytics.totalCount > 0
    ? (analytics.approvedCount / analytics.totalCount) * 100
    : 0;
  const pendingRate = analytics.totalCount > 0
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {analytics.totalCount} expense{analytics.totalCount !== 1 ? 's' : ''} recorded
            </p>
          </div>
          {/* Quick Status Badge */}
          <div className={cn('px-2.5 py-1 rounded-lg text-xs font-bold', statusBadge.className)}>
            {statusBadge.label}
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-amber-800">
                {analytics.pendingCount} expense{analytics.pendingCount !== 1 ? 's' : ''} awaiting review
              </p>
