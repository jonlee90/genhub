...
import { HardHat, Wrench, Hammer, ShieldAlert } from 'lucide-react';
...
interface ProjectListProps {
  projects: Project[];  // Pre-filtered by parent
  totalCount: number;   // For "X of Y" display
  onCreateClick?: () => void;  // For empty state button
  role?: string | null;
}

export function ProjectList({ projects, totalCount, onCreateClick, role }: ProjectListProps) {
    ...
    return (
      <div className="relative">
        ...
        <div className="relative flex flex-col items-center justify-center py-12 md:py-24 px-4 md:px-8 bg-gradient-to-br from-gray-50 via-white to-gray-50 rounded-xl md:rounded-2xl border-2 border-gray-200 shadow-construction-lg">
          ...
          {onCreateClick && (role === 'admin' || role === 'project_manager') && (
            <motion.div
              initial={{ y: 20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
