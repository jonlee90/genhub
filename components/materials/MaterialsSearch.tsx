...
export function MaterialsSearch({ projects }: MaterialsSearchProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Search Controls */}
      <div className="bg-white border-2 border-gray-200 rounded-lg p-6 shadow-construction">
        <div className="space-y-4">
          ...
          <div className="flex gap-4">
            <div className="relative flex-1">
              ...
              <Input
                type="text"
                placeholder="Search Home Depot products... (lumber, concrete, tools, etc.)"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                onKeyPress={handleKeyPress}
                className="pl-10 h-12 text-base border-2 focus:border-construction-blue"
              />
            </div>
            <Button
              onClick={handleSearch}
              disabled={isPending}
              className="h-12 px-6 bg-construction-blue hover:bg-construction-blue/90 text-white font-bold"
            >
              {isPending ? (
                <>
          ...
          <div className="flex flex-wrap gap-4 items-center">
            ...
            <div className="ml-auto flex gap-2">
              <Button
                ...
                className={viewMode === 'grid' ? 'bg-construction-blue' : ''}
              >
              ...
              <Button
                ...
                className={viewMode === 'list' ? 'bg-construction-blue' : ''}
              >
      ...
      <AnimatePresence>
        {selectedForComparison.length > 0 && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 20 }}
            className="bg-construction-blue text-white border-2 border-construction-blue rounded-lg p-4 shadow-construction-lg"
          >
      ...
      {products.length > 0 ? (
        ...
        <div className="flex items-center justify-center py-16">
          <div className="text-center space-y-4">
            <Loader2 className="h-12 w-12 animate-spin text-construction-blue mx-auto" />
            <p className="text-lg font-semibold text-gray-600">Searching Home Depot catalog...</p>
          </div>
        </div>
      ) : (
        <div className="bg-white border-2 border-dashed border-gray-300 rounded-lg p-12 text-center">
          <Search className="h-16 w-16 text-gray-400 mx-auto mb-4" />
          <h3 className="text-xl font-bold text-gray-900 mb-2">Search for Materials</h3>
