...
export function ProjectPreview({ id }: ProjectPreviewProps) {
  ...
  useEffect(() => {
    async function fetchProject() {
      ...
      try {
        const response = await fetch(`/api/chat/entity-preview?type=project&id=${id}`);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className={cn('flex flex-col items-center p-2 rounded-lg border-2', healthBgColor)}>
          <Activity className={cn('h-4 w-4 mb-1', healthColor)} />
          <span className={cn('text-xs font-black', healthColor)}>{project.health_score}%</span>
        </div>
      </div>
...
function getStatusVariant(status: string): { bg: string; text: string } {
  const variants: Record<string, { bg: string; text: string }> = {
    ...
    on_hold: { bg: 'bg-construction-yellow/20', text: 'text-construction-yellow' },
    completed: { bg: 'bg-construction-blue/20', text: 'text-construction-blue' },
