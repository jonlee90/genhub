  ...
  if (members.length === 0) {
    return (
      <div className="p-8 text-center">
        ...
        <p className="text-sm font-mono text-gray-500 dark:text-gray-400">No members found</p>
      </div>
    );
  }
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        "hover:border-construction-blue/20 dark:hover:border-construction-blue/40 hover:bg-construction-blue/5 dark:hover:bg-construction-blue/10",
        "transition-all duration-200",
      )}
    >
      {/* Debug: Avatar */}
      <Avatar className="h-10 w-10 shrink-0 border-2 border-gray-200 dark:border-gray-700">
...
function RoleIcon({ role }: { role: string }) {
  const icons: Record<string, React.ReactNode> = {
    ...
    subcontractor: <User className="h-5 w-5 text-gray-500 dark:text-gray-400" />,
  };

  return (
    <div className="p-2 bg-gray-50 dark:bg-gray-900 rounded-lg">
      {icons[role] || <User className="h-5 w-5 text-gray-400 dark:text-gray-500" />}
    </div>
  );
}
