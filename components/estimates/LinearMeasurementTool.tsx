...
/**
 * SVG-based polyline drawing tool for linear (distance) measurements.
 * Task: EST-P3-001-D
 *
...
export const LinearMeasurementTool = memo(function LinearMeasurementTool({
  scaleRatio,
  onComplete,
  onCancel,
  ...
  const handleSvgDoubleClick = useCallback(
    (e: React.MouseEvent<SVGSVGElement>) => {
      ...
      const lengthFeet =
        scaleRatio > 0
          ? scalePixelsToFeet(lengthPixels, scaleRatio)
          : lengthPixels;
      onComplete(points, { value: lengthFeet, unit: "ft" });
    },
  ...
  // Memoize polyline points string (rerender-memo)
  const polylinePoints = useMemo(
    () => points.map((p) => `${p.x},${p.y}`).join(" "),
    [points],
  );

  // Live length calculation for label
  const liveLength = useMemo(() => {
    ...
    const lengthFeet =
      scaleRatio > 0
        ? scalePixelsToFeet(lengthPixels, scaleRatio)
        : lengthPixels;
    return formatMeasurement(lengthFeet, "ft");
  }, [points, scaleRatio]);
  ...
  return (
    <div className="absolute inset-0 pointer-events-none">
      ...
      >
        ...
        {/* Live length label near last point */}
        {liveLength !== null && lastPoint !== null ? (
          <g filter="url(#linear-label-shadow)">
            <rect
              x={lastPoint.x + 12}
              y={lastPoint.y - 18}
              width={liveLength.length * 7 + 12}
              height={22}
              rx={4}
              fill="white"
            />
      ...
      <div className="absolute top-2 right-2 flex flex-col gap-2 pointer-events-auto">
        {/* Fix 2: Done button — visible when >= 2 points */}
        {canComplete ? (
          <button
            ...
            onClick={handleDone}
            aria-label="Complete linear measurement"
          >
            Done
          </button>
        ) : null}
