...
import type {
  TaskAssignee,
  AssigneeOption as TaskAssigneeOption,
} from "@/app/actions/tasks";
...
function TaskModalForm({
  ...
  const [primaryAssigneeId, setPrimaryAssigneeId] = useState<string | null>(
    null,
  );
  ...
  const [receiptPreview, setReceiptPreview] = useState<string | null>(() => {
    if (mode === "edit" && task?.receipt_photo_url)
      return task.receipt_photo_url;
    return null;
  });
      ...
      if (mode === "edit" && task?.assignees) {
        return task.assignees
          .map((a) => ({
            ...
            type: (a.user_id ? "user" : "subcontractor") as
              | "user"
              | "subcontractor",
          }))
        ...
        if (result?.error) {
          ...
          if (mode === "create" && result?.task && tempMaterials.length > 0) {
            try {
              ...
              const materialPromises = tempMaterials.map(
                async (tempMaterial) => {
  ...
  const approvalBadge = mode === "edit" &&
    ...
    config.styling.headerBadge === "approval_status" && (
      <span
        className={cn(
              ...
              <div className="p-4 rounded-xl border-2 border-amber-200 bg-amber-50">
                ...
                <div className="space-y-2 mb-3">
                  ...
                  <Textarea
                    ...
                    rows={2}
                    disabled={
                      isApprovalPending || task.approval_status === "approved"
                    }
