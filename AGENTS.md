...
## Multi-Agent Workflow: Claude Code + OpenCode GPT-5.2-Codex

This project uses a two-agent workflow for optimal code quality:

### Agent Responsibilities

| Agent | Role | Responsibilities |
|-------|------|------------------|
| **Claude Code** | Implementation | Feature development, bug fixes, new code |
| **OpenCode GPT-5.2-Codex** | Review & Refactor | Code review, reusability analysis, cleanup, debugging |
...
Claude Code (Implementation)
    ...
    │
    ▼
Creates handoff: .claude/handoffs/claude-to-opencode-{timestamp}.md
    │
    ▼
OpenCode GPT-5.2-Codex (Review)
    │
    ├── 1. Validates (tsc, lint, build)
    ├── 2. Scans for reusability opportunities
    ├── 3. Checks for code duplication
    ├── 4. Optimizes HTML/Tailwind
    ├── 5. Debugs and ensures 100% working
    │
    ▼
Creates response: .claude/handoffs/opencode-to-claude-{timestamp}.md
```
...
| Agent | Purpose |
|-------|---------|
| `reviewer` | Main code review and reusability analysis |
| `refactor` | Deep refactoring and component extraction |
| `component-scanner` | Module scanning for cleanup opportunities |
| `tailwind-optimizer` | HTML/CSS Tailwind class optimization |

### OpenCode Skills

| Skill | Purpose |
|-------|---------|
| `component-refactor-scanner` | Detect HTML soup, Tailwind issues |
| `reusable-code-detector` | Find patterns to extract |
| `vercel-react-best-practices` | Performance and pattern rules |

### Running OpenCode

```bash
# Review Claude's handoff
opencode run --agent reviewer --prompt "/review-handoff"

# Scan a module for refactoring
opencode run --agent component-scanner --prompt "/scan-module components/projects/"

# Deep refactor
opencode run --agent refactor --prompt "Refactor components/ui/Card.tsx"

# Tailwind optimization
opencode run --agent tailwind-optimizer --prompt "Optimize components/"
```

### Handoff Templates

See `.claude/handoffs/README.md` for complete handoff templates.
